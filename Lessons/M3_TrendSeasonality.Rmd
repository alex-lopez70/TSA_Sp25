...
data <- data.frame('t' = t, 'inflow.data' = inflow_data$HP8) # + 1 refers to date column

#Fit a linear trend to TS of iHP
linear.trend.model = lm(inflow.data~t, data)
summary(linear.trend.model)

beta0=as.numeric(linear.trend.model$coefficients[1])
beta1=as.numeric(linear.trend.model$coefficients[2])

linear.trend <- beta0 + beta1 * t
ts.lineartrend <- ts(linear.trend, start = c(1931,1), frequency = 12)
autoplot(ts_inflow_data, color = 'darkblue') +
  autolayer(ts.lineartrend, series = 'Linear Trend')

detrend.inflow <- inflow_data$HP8 - linear.trend
ts.detrend <- ts(detrend.inflow, start = c(1931,1), frequency = 12)

autoplot(ts_inflow_data, color = 'darkblue') +
  autolayer(ts.detrend, series = 'Detrended Series')
